...
\begin{enumerate}
  ...
  \item \textbf{The datapath-degradation principle}, which predicts both orderings where state
    fidelity alone does not, with the quantizer contrast as an out-of-sample (post-hoc
    identified) test (\S\ref{sec:principle}).
...
parameters, short training schedules, two tasks --- and every energy number is a 45\,nm
operation-count proxy, not a measurement on silicon. The analog storage element and its converter,
which are real area and power costs, are not priced by that proxy at all. We state these limits
