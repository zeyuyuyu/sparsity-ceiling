...
**Addendum 2026-08-04 — the model-shape caveat above is now measured, and it closes the last open door: the "8% term" statement is a language-model-shape fact, and at the shape where the recurrence DOES dominate, the energy is recoverable but the quality is not.** Sweeping the same accounting (nothing re-derived) over readout width V at fixed E=64, H=256 and the measured char-LM activity: at V=284 (V/H=1.11) the route is worth 1.60× (ceiling 1.74×); at a word-piece vocabulary (V=32k) it is worth **1.01× — nothing**; at a small-output streaming shape (V=10, V/H=0.04) the readout falls to 9.5%, the recurrence becomes the **largest** single term (29.2%), and the same measured activity projects **3.16×**, growing with H (5.58× at H=1024) and to 6.21× with an event-priced input. So "sparsifying an SSM recurrence optimizes an 8% term" needs the qualifier **at a language-model shape**. We then measured quality at the favourable shape (pre-registered criteria; FashionMNIST 28-step row streaming, V=10, chance 0.10). The **energy** projection confirmed out of sample — measured **3.29×**, and the digital decomposition inverts exactly as predicted (recurrence 75.7%, input 21.0%, readout 3.0%). The **quality** side was refuted: the best analog setting keeps 86% of the noise-regularized reference's accuracy (0.677 vs 0.788), an 11-point deficit, and the pre-registered >5-point refutation criterion is met at every threshold (11–58 points). A pre-registered budget check (30 epochs, full 60k train) leaves the deficit flat (−10.4/−18.9 points vs a 0.844 reference) while both arms improve absolutely — so this is a property of the lossy analog state at this shape, not an under-trained baseline. Three notes that make it credible rather than merely disappointing: (i) the 3.3–4.0× band is **shape-determined, not mechanism-determined** — all three neuromorphic variants land in it, so no mechanism can claim the factor; (ii) priced conservatively (graded events as MACs) analog falls to 1.77× while LIF output-spiking keeps an unambiguous **3.34× at only 2.9 points worse** — if anyone builds at this shape, the defensible design is a **spiked output on an exact state**, not an analog state; (iii) our own datapath-degradation principle, used as a forecast, predicted the opposite outcome here and has now mispredicted twice (the event readout on precise recall was the first) — treat it as a post-hoc ordering of measured degradations, not a predictive law. **Bottom line, now unqualified: across every model shape and training budget we tested there is no quality-matched pJ win — where quality survives, the energy is stranded (language shape); where the energy is recoverable, quality fails (streaming shape).** (Caveats: streaming cells are single-seed — effects of 10–19 points against ≤1-point seed noise elsewhere in this pipeline; input is dense, so the 6.21× event-input corner is untested, but it improves only the energy axis, which is not the axis that failed; all energy is the 45 nm proxy.)
